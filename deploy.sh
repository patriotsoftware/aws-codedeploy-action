...
function deployRevision() {
    aws deploy create-deployment "$@" \
        ...
        --s3-location bucket="$INPUT_S3_BUCKET",bundleType="zip",key="$INPUT_ARCHIVE" | jq -r '.deploymentId'
}
...
function pollForSpecificDeployment() {
    ...
    while true; do
        ...
        echo -e "${ORANGE}Deployment in progress. Sleeping 45 seconds. (Try $((++deadlockCounter)))";
